...
const FlashLightActionNode: NodeDefinition = {
  ...
  execute: async (context: NodeExecutionContext): Promise<NodeExecutionResult> => {
    try {
      ...
      logger.info(
        `[FlashLightAction ${context.nodeId}] Initializing with mode: ${mode}, value: ${value}, propagate: ${propagateSignal}`
      );
      ...
      logger.debug(
        `[FlashLightAction ${context.nodeId}] current signalSystem:`,
        signalSystem ? 'present' : 'missing'
      );
      ...
      signalSystem.registerHandler(context.nodeId, async (signal: any): Promise<any> => {
        try {
          ...
          if (mode === 'set') {
            ...
            logger.info(
              `[FlashLightAction ${context.nodeId}] Mode TOGGLE: ${currentState} -> ${newState}`
            );
          }
          ...
          logger.info(
            `[FlashLightAction ${context.nodeId}] ensureCameraPermission returned: ${permissionOk}`
          );
          logger.info(
            `[FlashLightAction ${context.nodeId}] Camera permission result: ${permissionOk}`
          );
          if (!permissionOk) {
            logger.warn(
              `[FlashLightAction ${context.nodeId}] Permission denied - emitting flashlight.permission.failed`
            );
            // Emit event so the app can react (UI banner, etc.)
            try {
              logger.info(
                `[FlashLightAction ${context.nodeId}] Emitting event flashlight.permission.failed`
              );
              const ss2 = require('../SignalSystem').getSignalSystem();
              if (ss2)
                ss2.emitEvent('flashlight.permission.failed', {
                  nodeId: context.nodeId,
                  timestamp: Date.now(),
                });
            } catch (e) {
              logger.warn(
                `[FlashLightAction ${context.nodeId}] Failed to emit flashlight.permission.failed`,
                e
              );
            }
            ...
            try {
              if (signalSystem) {
                logger.info(
                  `[FlashLightAction ${context.nodeId}] Emitting permission failed via signalSystem`
                );
                ...
                try {
                  (signalSystem as any).stats.lastEmittedEvent = 'flashlight.permission.failed';
                } catch {
                  /* ignore */
                }
              }
          ...
          logger.info(
            `[FlashLightAction ${context.nodeId}] Calling setFlashlightState(${newState})...`
          );

          try {
            await setFlashlightState(newState);
            logger.info(
              `[FlashLightAction ${context.nodeId}] ✅ Flashlight state changed successfully!`
            );
          } catch (flashError) {
            logger.error(
              `[FlashLightAction ${context.nodeId}] Failed to change flashlight:`,
              flashError
            );
            Alert.alert(
              'Erreur',
