    ...
    <div className="min-h-screen bg-linear-to-br from-slate-950 via-slate-900 to-sky-900 text-white flex items-center justify-center px-4">
      <div className="max-w-lg w-full bg-white/5 border border-white/10 rounded-2xl p-8 backdrop-blur shadow-2xl text-center space-y-4">
        <div className={`mx-auto h-14 w-14 rounded-full flex items-center justify-center ${
          status === "loading" ? "bg-sky-500/20" : status === "success" ? "bg-emerald-500/20" : "bg-red-500/20"
        }`}>
          {status === "loading" && (
            <div className="h-6 w-6 border-2 border-white/30 border-t-white rounded-full animate-spin" aria-label="Chargement" />
          )}
        ...
        {status === "error" && (
          <div className="space-y-2 text-sm text-slate-300">
            ...
            <Link href={`/login?callbackUrl=${encodeURIComponent("/mods")}`} className="text-sky-200 underline">
