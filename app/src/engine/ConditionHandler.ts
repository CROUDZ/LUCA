...
export async function activateOutput(
  ...
  logger.info(`[ConditionHandler] Node ${nodeId} OUTPUT ON (timer=${timerDuration}s, switch=${switchMode})`);
...
export async function onConditionUnmet(
  ...
  if (timerDuration === 0 && state.isOutputActive && state.hasActiveSignal) {
    logger.info(`[ConditionHandler] Node ${nodeId} condition UNMET, deactivating (continuous mode)`);
    await deactivateOutput(nodeId, callbacks);
  }
...
export function createConditionNode(config: ConditionNodeConfig): NodeDefinition {
  ...
  return {
    ...
    generateHTML: (settings: Record<string, any>, nodeMeta?: NodeMeta): string => {
      ...
      const body = `
        <div class="condition-node${invertSignal ? ' inverted' : ''}${switchMode ? ' switch-mode' : ''}">
