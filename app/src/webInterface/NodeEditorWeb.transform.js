...
function getPrecanvas() {
  ...
  const fallback =
    container.querySelector('.parent-drawflow') || container.querySelector('.drawflow');
  if (fallback) {
...
function zoomBySteps(stepCount) {
  ...
  const factor =
    stepCount > 0
      ? Math.pow(BUTTON_ZOOM_FACTOR, stepCount)
      : Math.pow(1 / BUTTON_ZOOM_FACTOR, Math.abs(stepCount));
...
function isTouchOnControl(touch) {
  ...
  return (
    ...
    el.closest &&
    el.closest(
      '.delay-control, .condition-invert-control, .logic-gate-control, input, select, button'
    ) != null
  );
}
...
function initTransformListeners() {
  ...
  window.DrawflowEditor.debugLog &&
    window.DrawflowEditor.debugLog('[NodeEditorWeb] Transform listeners initialized');
}
