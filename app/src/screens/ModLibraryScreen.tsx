...
const ModLibraryScreen: React.FC<ModLibraryScreenProps> = ({ navigation }) => {
  ...
  const fetchMods = useCallback(async (pageNum: number = 1, refresh: boolean = false) => {
    try {
      if (refresh) {
        setRefreshing(true);
      } else if (pageNum === 1) {
  ...
  const handleInstallMod = useCallback(async (mod: Mod) => {
    ...
    if (isInstalled) {
      ...
      if (!mod.verified) {
        ...
        Alert.alert(
          'Installer le mod',
          `Voulez-vous installer "${mod.displayName}" ?`,
          [
            { text: 'Annuler', style: 'cancel' },
            {
              text: 'Installer',
  ...
  const renderModCard = useCallback(({ item }: { item: Mod }) => {
      ...
      >
        ...
        <View style={styles.modInfo}>
          <View style={styles.modNameRow}>
            <Text style={styles.modName} numberOfLines={1}>{item.displayName}</Text>
            {isInstalled && (
          ...
          <Text style={styles.modAuthor} numberOfLines={1}>par {item.author.name}</Text>
        ...
        >
          {isCurrentlyInstalling ? (
            ...
            <Icon 
              name={isInstalled ? "delete" : "download"} 
              size={20} 
              color="#fff" 
            />
          )}
    ...
    <View style={styles.header}>
      ...
      <View style={styles.searchContainer}>
        ...
        {searchQuery.length > 0 && (
          <TouchableOpacity onPress={() => { setSearchQuery(''); fetchMods(1); }}>
