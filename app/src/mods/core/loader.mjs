  ...
  async forkRunner(mod) {
    return new Promise((resolve, reject) => {
      ...
      childProcess.on('exit', (code, signal) => {
        console.log(`[loader] Runner for ${mod.manifest.name} exited (code: ${code}, signal: ${signal})`);
  ...
  handleRunnerExit(modName, code, signal) {
    ...
    if (mod.status === 'active' && code !== 0) {
      ...
      if (mod.restartCount < this.config.maxRestarts) {
        mod.restartCount++;
        console.log(`[loader] Attempting restart ${mod.restartCount}/${this.config.maxRestarts} for ${modName}`);
