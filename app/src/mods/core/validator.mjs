...
const DANGEROUS_PATTERNS = [
  ...
  { pattern: /require\s*\(\s*['"]child_process['"]\s*\)/, severity: 'critical', message: 'child_process import not allowed' },
  { pattern: /import\s+.*from\s+['"]child_process['"]/, severity: 'critical', message: 'child_process import not allowed' },
  { pattern: /require\s*\(\s*['"]fs['"]\s*\)/, severity: 'critical', message: 'Direct fs import not allowed (use storage API)' },
  { pattern: /import\s+.*from\s+['"]fs['"]/, severity: 'critical', message: 'Direct fs import not allowed (use storage API)' },
  { pattern: /import\s+.*from\s+['"]fs\/promises['"]/, severity: 'critical', message: 'Direct fs/promises import not allowed' },
  { pattern: /require\s*\(\s*['"]net['"]\s*\)/, severity: 'critical', message: 'net module not allowed' },
  { pattern: /require\s*\(\s*['"]dgram['"]\s*\)/, severity: 'critical', message: 'dgram module not allowed' },
  { pattern: /require\s*\(\s*['"]cluster['"]\s*\)/, severity: 'critical', message: 'cluster module not allowed' },
  { pattern: /require\s*\(\s*['"]worker_threads['"]\s*\)/, severity: 'critical', message: 'worker_threads not allowed' },
  { pattern: /require\s*\(\s*['"]vm['"]\s*\)/, severity: 'critical', message: 'vm module not allowed' },
  
  // Exécution dynamique
  { pattern: /\beval\s*\(/, severity: 'critical', message: 'eval() is not allowed' },
  { pattern: /new\s+Function\s*\(/, severity: 'critical', message: 'new Function() is not allowed' },
  { pattern: /setTimeout\s*\(\s*['"`]/, severity: 'high', message: 'setTimeout with string argument is dangerous' },
  { pattern: /setInterval\s*\(\s*['"`]/, severity: 'high', message: 'setInterval with string argument is dangerous' },
  ...
  { pattern: /process\.env/, severity: 'medium', message: 'process.env access may leak sensitive data' },
  { pattern: /process\.execPath/, severity: 'high', message: 'process.execPath access not allowed' },
  ...
  { pattern: /globalThis\s*\[/, severity: 'high', message: 'Dynamic globalThis access is dangerous' },
  { pattern: /__proto__/, severity: 'high', message: '__proto__ manipulation not allowed' },
  { pattern: /Object\.setPrototypeOf/, severity: 'medium', message: 'Prototype manipulation is discouraged' },
  
  // Réseau non autorisé
  { pattern: /require\s*\(\s*['"]http['"]/, severity: 'high', message: 'Direct http module not allowed (use api.http)' },
  { pattern: /require\s*\(\s*['"]https['"]/, severity: 'high', message: 'Direct https module not allowed (use api.http)' },
  { pattern: /fetch\s*\(/, severity: 'medium', message: 'Direct fetch() - ensure network.http permission is declared' },
];
...
async function validateManifest(manifest, modPath) {
  ...
  if (manifest.manifest_version !== 1) {
    result.addError('INVALID_MANIFEST_VERSION', `manifest_version must be 1`, { got: manifest.manifest_version });
  }
  
  // Valider le nom (alphanumeric + hyphens)
  if (manifest.name && !/^[a-z0-9-]+$/.test(manifest.name)) {
    result.addError('INVALID_NAME', 'name must be lowercase alphanumeric with hyphens only', { name: manifest.name });
  }
  
  // Valider la version (semver)
  if (manifest.version && !/^\d+\.\d+\.\d+(-[\w.]+)?(\+[\w.]+)?$/.test(manifest.version)) {
    result.addError('INVALID_VERSION', 'version must be valid semver', { version: manifest.version });
  }
  
  // Valider api_version (semver)
  if (manifest.api_version && !/^\d+\.\d+\.\d+$/.test(manifest.api_version)) {
    result.addError('INVALID_API_VERSION', 'api_version must be valid semver', { api_version: manifest.api_version });
  }
    ...
    if (!Array.isArray(manifest.permissions)) {
      ...
      for (const perm of manifest.permissions) {
        if (!VALID_PERMISSIONS.includes(perm)) {
          result.addWarning('UNKNOWN_PERMISSION', `Unknown permission: ${perm}`, { permission: perm });
        }
      }
    }
  }
  ...
  if (manifest.main) {
    ...
    if (!existsSync(mainPath)) {
      result.addError('MAIN_NOT_FOUND', `Main file not found: ${manifest.main}`, { path: mainPath });
    }
  }
...
function isWhitelistedModule(moduleName) {
  ...
  return WHITELISTED_MODULES.some(w => 
    moduleName === w || moduleName.startsWith(w + '/')
  );
}
