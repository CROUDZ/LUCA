...
describe('Manifest Validation', () => {
  ...
  it('should fail on missing required fields', async () => {
    await writeFile(path.join(testModPath, 'manifest.json'), JSON.stringify({
      name: 'test-mod'
...
describe('Security Scan', () => {
  ...
  it('should detect eval() usage', async () => {
    await writeFile(path.join(testModPath, 'main.mjs'), `
    ...
    expect(result.errors.some(e => 
      e.code === 'DANGEROUS_CODE' && e.message.includes('eval')
    )).toBe(true);
  });
  ...
  it('should detect child_process import', async () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.errors.some(e => 
      e.code === 'DANGEROUS_CODE' && e.message.includes('child_process')
    )).toBe(true);
  });
  ...
  it('should detect fs import', async () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.errors.some(e => 
      e.code === 'DANGEROUS_CODE' && e.message.includes('fs')
    )).toBe(true);
  });
  ...
  it('should detect process.exit', async () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.errors.some(e => 
      e.code === 'DANGEROUS_CODE' && e.message.includes('process.exit')
    )).toBe(true);
  });
  ...
  it('should allow safe code', async () => {
    ...
    const criticalErrors = result.errors.filter(e => 
      e.code === 'DANGEROUS_CODE' && e.severity === 'critical'
    );
