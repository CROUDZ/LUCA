...
app.get('/api/mods', (req, res) => {
  try {
    ...
    const sortColumn = ['downloads', 'rating', 'updated_at', 'name'].includes(sort) ? sort : 'downloads';
...
app.get('/api/mods/:name/download', (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="${mod.name}-${version || mod.version}.zip"`);
    ...
    paths: {
      '/api/mods': {
        get: {
          ...
          parameters: [
            ...
            { name: 'sort', in: 'query', schema: { type: 'string', enum: ['downloads', 'rating', 'updated_at', 'name'] } },
      ...
      '/api/mods/upload': {
        post: {
          summary: 'Upload mod',
          security: [{ bearerAuth: [] }],
          requestBody: { content: { 'multipart/form-data': { schema: { type: 'object', properties: { mod: { type: 'string', format: 'binary' } } } } } },
