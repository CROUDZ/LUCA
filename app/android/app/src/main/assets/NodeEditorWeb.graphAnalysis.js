...
function analyzeGraphImpl() {
  ...
  if (hasCycle) {
    const allConnected = new Set(
      nodeIds.filter((id) => graph[id].length > 0 || reverseGraph[id].length > 0)
    );
...
function ensureGraphExportListeners(attempt = 0) {
  ...
  if (attempt > 20) {
    console.warn(
      '[NodeEditorWeb] Impossible de lier les événements Drawflow - export automatique désactivé'
    );
    return;
  }
